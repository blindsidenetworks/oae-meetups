...
var joinURL = module.exports.joinURL = function(ctx, meetingProfile, configXML, callback) {
    ...
    BBBProxy.executeBBBCall(meetingInfoURL, function(err, meetingInfo) {
        ...
        if ( meetingInfo.returncode === 'FAILED' && meetingInfo.messageKey === 'notFound' ) {
            ...
            // Create the meeting
            var paramsGroup = {'meetingID': meetingID, 'name':meetingName, 'logoutURL': logoutURL, 'record': record};
            if (meetingProfile.resourceType === 'group') {
                ...
                paramsGroup['meta_bn-recording-ready-url'] = baseUrl+'/api/meetup/'+meetingProfile.id+'/recording';
            }
            var createMeetingURL = _getBBBActionURL(bbbConfig.endpoint, 'create', bbbConfig.secret, _getQueryStringParams(paramsGroup));
            return BBBProxy.executeBBBCall(createMeetingURL, function(err, meetingInfo) {
                if (err) {
                    return callback(err);
                }
                ...
                var paramsSign = {'meetingID': meetingID, 'fullName':fullName, 'password': password};
                return _signURL(paramsSign, bbbConfig, configXML, callback);
            });
        } else {
            // Construct and sign the URL
            var password = _getJoiningPassword(meetingProfile, meetingInfo);
...
var _signURL = function(params, bbbConfig, configXML, callback) {

    if(configXML) {
        var config_xml_params = _getSetConfigXMLParams(bbbConfig.secret, params['meetingID'], configXML);
        var setConfigXMLURL = bbbConfig.endpoint + 'api/setConfigXML';
        log().info(setConfigXMLURL);
        return BBBProxy.executeBBBCallExtended(setConfigXMLURL, null, 'post', config_xml_params, 'application/x-www-form-urlencoded', function(err, response) {
...
var getRecordingsURL = module.exports.getRecordingsURL = function(ctx, profile, callback) {
    ...
    MeetupsDAO.getMeetup(meetingID, function (err, meeting){
        if (err) {
            ...
            var meetup = meeting;
            return PrincipalsUtil.getUser(ctx, meetup.createdBy, function (err, user){
                if (err) {
                    return callback(err);
                } else {
...
var _getJoiningPassword = function(profile, meetingInfo) {
    ...
    if ( profile.isManager || profile.allModerators === 'true' ) {
        return password = meetingInfo.moderatorPW;
    }
    return password = meetingInfo.attendeePW;
}
